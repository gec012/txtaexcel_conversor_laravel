...
<div class="card-heading bg-success  " style="text-align:center;"><h2 class="card-title"> Informes afiliados activos  sin aportes en el sistema SGAP</h2></div>
...
<table id="myTable" class="table table-hover table-active table-bordered table-responsive table-condesed">
    ...
    @foreach ($archivos as $arch)
        <tr class="text-white">
            ...
            @if($arch->extension == 'xlsx')
                <td><a href="{{$arch->url}}" target='_blank'>Ver Archivo Excel</a></td>
            @else
                <td><a href="{{$arch->url}}" target='_blank'>Ver Archivo Word</a></td>
            @endif
             <td>{{ $arch->nombre}}</td>
            ...
            <td> {!! Form::open(['method' => 'DELETE','route' => ['reportes_activos_sa.destroy',$arch->id],'style'=>'display:inline']) !!}
